...
def find_training(expr, offspr, the_country):
	res = {}
	try:
		# ---init
		f = file("WDDB_utf8", 'rb')
		...
		table.setc(0, 1, "<i><u><b>GENDER</b></u></i>")
		table.setc(0, 2, "<i><u><b>MOTHER</b></u></i>")
		table.setc(0, 3, "<i><u><b>FATHER</b></u></i>")
		table.setc(0, 4, "<i><u><b>TRAINING</b></u></i>")
		table.setc(0, 5, "<i><u><b>HIP</b></u></i>")
		table.setc(0, 6, "<i><u><b>ED</b></u></i>")

		table.setc(0, 7, "<i><u><b>BONITATION</b></u></i>")
		table.setc(0, 8, "<i><u><b>BIRTHDAY</b></u></i>")
		table.setc(0, 9, "<i><u><b>BREEDER</b></u></i>")
		table.setc(0, 10, "<i><u><b>ORIGIN COUNTRY</b></u></i>")
		...
		if expr[:6] == "regex:":
			...
			for elem in sorted_list:
				...
				cond = p.search(dump_dict[key]["TRAINING"].lower())

				# offspring
				if offspr == "True":
					cond = cond and dump_dict[key].has_key("OFFSPRING")

				# or cond on training code
				if cond:
					...
					table.setc(i+line, 4, dump_dict[key]["TRAINING"].replace("data not available", "?"))
					...
					if dump_dict[key]["BIRTHDAY"]!="data not available":
						table.setc(i+line, 8, "<a href=%s/find_birth.py?year=%s>%s</a>"%(SERVER, dump_dict[key]["BIRTHDAY"],dump_dict[key]["BIRTHDAY"]))
					else:
						table.setc(i+line, 8, dump_dict[key]["BIRTHDAY"].replace("data not available", "?"))

					#table.setc(i+line, 6, dump_dict[key]["BREEDER"].replace("data not available", "?"))
					if dump_dict[key]["BREEDER"]!="data not available":
						table.setc(i+line, 9, "<a href=%s/find_breeders.py?name=%s>%s</a>"%(SERVER, dump_dict[key]["BREEDER"].replace(" ", "%20" ), dump_dict[key]["BREEDER"].replace("data not available", "?")))
					else:
			...
			if or_expr_list:
				...
				for elem in sorted_list:
					...
					for rex in or_expr_list:
						cond = cond or re.search(rex.lower().replace(" ", ""), dump_dict[key]["TRAINING"].lower())
			...
			if and_expr_list:
				...
				for elem in sorted_list:
					...
					for rex in and_expr_list:
						cond = cond and re.search(rex.lower().replace(" ", ""), dump_dict[key]["TRAINING"].lower())
						
					# offspring
					if offspr == "True":
						cond = cond and dump_dict[key].has_key("OFFSPRING")

					# and cond on training code
					if cond:		
						if IMAGE_DISPLAY:
							if os.path.isfile("pics/%s"%key.replace(".html", ".jpg")):
								table.setc(i+line, 0, "<u><a href=%s>%s</a></u>"%(os.path.join("pics", key.replace(".html", ".jpg")),dump_dict[key]["NAME"]))
		...
		if pos_or<0 and pos_and<0:
			...
			for elem in sorted_list:
				...
				cond = re.search(expr.lower(), dump_dict[key]["TRAINING"].lower())
		...
		print "Error in find_training: %s"%e
...
try:
	...
	if form.has_key("expr"):
		...
		if form.has_key("offspring"):
			...
			s = find_training(expr, offspring, country)
		else:
			country = form["country"].value
			s = find_training(expr, False, country)
	else:
