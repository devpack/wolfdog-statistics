...
def find_hd(hdstr, offspr, the_country):
	...
	try:
		...
		table.setc(3, 7, "<i><u><b>BREEDER</b></u></i>")
		...
		for elem in sorted_list:
			...
			hd_str = dump_dict[key]["HIP"].lower().replace("data not available", "")
			cond = re.search(hdstr.lower(), hd_str[:1])
			...
			if cond:
				...
				if dump_dict[key]["BREEDER"]!="data not available":
					table.setc(i+line, 7, "<a href=%s/find_breeders.py?name=%s>%s</a>"%(SERVER, dump_dict[key]["BREEDER"].replace(" ", "%20"), dump_dict[key]["BREEDER"]))
				else:
					table.setc(i+line, 7, dump_dict[key]["BREEDER"].replace("data not available", "?"))
...
try:
	...
	if form.has_key("hd"):
		...
		if form.has_key("offspring"):
			...
			s = find_hd(hd, offspring, country)

		else:
			country = form["country"].value
			s = find_hd(hd, False, country)
