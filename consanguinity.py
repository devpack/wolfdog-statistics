...
class Tool(object):
	def __init__(self, limit, from_year):
		...
		self.table.setc(START, 1, "<i><u><b>COI5G</b></u></i>")
		self.table.setc(START, 2, "<i><u><b>COI8G</b></u></i>")
	...
	def go(self):
			...
			if self.dump_dict[key]["BIRTHDAY"] != "data not available":
				...
				try:
					...
					if birth > self.FROM_YEAR:
						result.append((float(self.dump_dict[key]["COIFA"][:-1]), key))
				except:
					pass
		...
		for el in result[:self.LIMIT]:
			...
			self.table.setc(1+i, 1, self.dump_dict[key]["COI5G"])
			self.table.setc(1+i, 2, self.dump_dict[key]["COIFA"])
