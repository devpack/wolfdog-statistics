...
class Tool(object):
	...
	def htmlprint_stat(self):
		try:
			if not self.mix:
				...
				self.table.setc(1, 3, self.dump_dict[self.DOG_ID]["BONITATION"].replace("data not available", "?"))
				...
				self.table.setc(1, 7, self.dump_dict[self.DOG_ID]["ED"].replace("data not available", "?"))

				self.table.setc(1, 8, self.dump_dict[self.DOG_ID]["COI5G"].replace("data not available", "?"))
			...
			# ---------- mix
				...
				self.table.setc(1, 3, self.dump_dict[self.DOG_ID1]["BONITATION"].replace("data not available", "?"))
				...
				self.table.setc(1, 7, self.dump_dict[self.DOG_ID1]["ED"].replace("data not available", "?"))

				self.table.setc(1, 8, self.dump_dict[self.DOG_ID1]["COI5G"].replace("data not available", "?"))
				...
				self.table.setc(2, 3, self.dump_dict[self.DOG_ID2]["BONITATION"].replace("data not available", "?"))
				...
				self.table.setc(2, 7, self.dump_dict[self.DOG_ID2]["ED"].replace("data not available", "?"))

				self.table.setc(2, 8, self.dump_dict[self.DOG_ID2]["COI5G"].replace("data not available", "?"))
			...
			if not self.mix:
				self.table.setc(START+1, 1, "<h3><b>%s%%</b></h3>"%self.wp(self.DOG_ID, 0))
			...
			o=0
			for elem in wl:
				self.table.setc(START+3+o, 0, "%s"%elem[1])
				self.table.setc(START+3+o, 1, "%s%%"%str(elem[0]*100./wsum))
				o+=1
	...
	def iswolf(self, dog_id, deep=1):
		if self.dump_dict[dog_id]["NAME"] == "FLEUR WOLVIN":
			if self.wolf_proportion.has_key("FLEUR WOLVIN"):
				self.wolf_proportion["FLEUR WOLVIN"] += 1./pow(2, deep)
			else:
				self.wolf_proportion["FLEUR WOLVIN"] = 1./pow(2, deep)
			return True

		if self.dump_dict[dog_id]["NAME"] == "FLEUR II WOLVIN":
			if self.wolf_proportion.has_key("FLEUR II WOLVIN"):
				self.wolf_proportion["FLEUR II WOLVIN"] += 1./pow(2, deep)
			else:
				self.wolf_proportion["FLEUR II WOLVIN"] = 1./pow(2, deep)
			return True

		if self.dump_dict[dog_id]["NAME"] == "WOLVIN":
			if self.wolf_proportion.has_key("WOLVIN"):
				self.wolf_proportion["WOLVIN"] += 1./pow(2, deep)
			else:
				self.wolf_proportion["WOLVIN"] = 1./pow(2, deep)
			return True
