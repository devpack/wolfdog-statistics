...
class Tool(object):
	def __init__(self, limit, depth, from_year):
		...
		self.table.setc(0, 5, "<i><u><b>BONITATION</b></u></i>")
		self.table.setc(0, 6, "<i><u><b>BREEDER</b></u></i>")
		self.table.setc(0, 7, "<i><u><b>ORIGIN COUNTRY</b></u></i>")
	...
	def go(self):
		...
		for el in result:
			...
			if IMAGE_DISPLAY:
				...
				self.table.setc(i+1, 0, "<a href=%s/form.py?id=%s>%s</a>"%(SERVER, el[1].split(".")[0][1:], self.dump_dict[el[1]]["NAME"]))
			...
			if self.dump_dict[el[1]]["BIRTHDAY"]!="data not available":
				self.table.setc(i+1, 3, "<a href=%s/find_birth.py?year=%s>%s</a>"%(SERVER, self.dump_dict[el[1]]["BIRTHDAY"],self.dump_dict[el[1]]["BIRTHDAY"]))
			else:
			...
			self.table.setc(1+i, 5, self.dump_dict[el[1]]["BONITATION"].replace("data not available", "?"))

			if self.dump_dict[el[1]]["BREEDER"]!="data not available":
				self.table.setc(i+1, 6, "<a href=%s/find_breeders.py?name=%s>%s</a>"%(SERVER, self.dump_dict[el[1]]["BREEDER"].replace(" ", "%20" ), self.dump_dict[el[1]]["BREEDER"]))
			else:
				self.table.setc(i+1, 6, self.dump_dict[el[1]]["BREEDER"].replace("data not available", "?"))

			self.table.setc(1+i, 7, self.dump_dict[el[1]]["ORIGIN COUNTRY"].replace("data not available", "?"))
