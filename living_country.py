...
class Tool(object):
	...
	def go(self):
		...
		for key in self.dump_dict:
			if self.dump_dict[key]["LIVING COUNTRY"].lower() == self.country_name:
		...
		self.table.setc(1, 0, "%s%s: TOTAL=%s"%(self.country_name[0].upper(), self.country_name[1:], str(j)))
...
def do_nb_offspring(country_name):
	try:
		T = Tool(country_name)
		T.go()
		return T.get_result()
...
try:
	...
	if form.has_key("country"):
		...
		s = do_nb_offspring(country_name)
	else:
	...
	print "Content-Type: text/html\n"
	print "Error in living country: %s"%e
