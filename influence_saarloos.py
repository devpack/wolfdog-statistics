...
class Tool(object):
	...
	def inf(self):
		...
		for key in res_weight:
			try:
				new_res.append((res_weight[key], self.dump_dict[key]["BIRTHDAY"], self.dump_dict[key]["NAME"], res_nb[key]))
			except Exception, e:
		...
		print "Content-Type: text/html\n"
		for elem in new_res:
			if elem[1]=="data not available":
				print "%s%%"%((float(elem[0])*100)/sum), " -- ", "xxxx.xx.xx" , " -- ", elem[2], "(weight: %s)"%elem[0], "(nb: %s)"%elem[3]
			else:
				print "%s%%"%((float(elem[0])*100)/sum), " -- ", elem[1] , " -- ", elem[2], "(weight: %s)"%elem[0], "(nb: %s)"%elem[3]
	...
	def htmlprint_stat(self):
		try:
			...
			self.table.setc(START+1, 3, "<i><u>influence</u></i>")
