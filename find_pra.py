...
def find_pra(hdstr, offspr, the_country):
	res = {}
	try:
		# ---init
		f = file("WDDB_utf8", 'rb')
		...
		table.setc(3, 9, "<i><u><b>BIRTHDAY</b></u></i>")
		table.setc(3, 10, "<i><u><b>BREEDER</b></u></i>")
		...
		for elem in sorted_list:
			...
			if hdstr.lower() == "free":
				pra_str = dump_dict[key]["PRA"].lower().replace("data not available", "")
				pos = pra_str.find("free")
				if pos >= 0:
					cond = True
				else:
					cond = False 

			if hdstr.lower() == "non free":
				if dump_dict[key]["PRA"].lower() != "data not available":
					pra_str = dump_dict[key]["PRA"].lower().replace("data not available", "")
					pos = pra_str.find("free")
					if pos < 0:
						cond = True
			...
			if cond:
				...
				if dump_dict[key]["BIRTHDAY"]!="data not available":
					table.setc(i+line, 9, "<a href=%s/find_birth.py?year=%s>%s</a>"%(SERVER, dump_dict[key]["BIRTHDAY"],dump_dict[key]["BIRTHDAY"]))
				else:
					table.setc(i+line, 9, dump_dict[key]["BIRTHDAY"].replace("data not available", "?"))


				if dump_dict[key]["BREEDER"]!="data not available":
					table.setc(i+line, 10, "<a href=%s/find_breeders.py?name=%s>%s</a>"%(SERVER, dump_dict[key]["BREEDER"].replace(" ", "%20"), dump_dict[key]["BREEDER"]))
				else:
...
try:
	...
	if form.has_key("pra"):
		...
		if form.has_key("offspring"):
			...
			s = find_pra(pra, offspring, country)

		else:
			country = form["country"].value
			s = find_pra(pra, False, country)
