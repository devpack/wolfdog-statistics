...
class Tool(object):
	...
	def htmlprint_stat(self):
		try:
			...
			if not self.mix:
				DOG_ID_MOTHER = self.dump_dict["d%s.html"%self.DOG_ID]["MOTHER"]
				DOG_ID_FATHER = self.dump_dict["d%s.html"%self.DOG_ID]["FATHER"]
			else:
				#DOG_ID_MOTHER = self.dump_dict["d%s.html"%self.DOG_ID1]["MOTHER"]
				#DOG_ID_FATHER = self.dump_dict["d%s.html"%self.DOG_ID2]["FATHER"]
				DOG_ID_MOTHER = "d%s.html"%self.DOG_ID1
				DOG_ID_FATHER = "d%s.html"%self.DOG_ID2
			...
			new_res = []
			for key in self.res:
				new_res.append( (self.res[key]*100, key) )
			
			new_res.sort(reverse=True)


			START = 4
			self.table.setc(START+1, 0, "<center><h3><b>COI</b></h3><i><h5>[Sum (1/2)^n1+n2+1]<h5></i></center>")
			...
			self.table.setc(START+2, 6, "<i><u>Bonitation</u></i>")
			self.table.setc(START+2, 7, "<i><u>Idx H</u></i>")
			self.table.setc(START+2, 8, "<i><u>Idx F</u></i>")
			self.table.setc(START+2, 9, "<i><u>HD</u></i>")
			self.table.setc(START+2, 10, "<i><u>ED</u></i>")
			self.table.setc(START+2, 11, "<i><u>COI5G</u></i>")
			...
			for elem in new_res:
				...
				if self.dump_dict[key]["BIRTHDAY"]!="data not available":
					self.table.setc(i+line, 5, "<a href=%s/find_birth.py?year=%s>%s</a>"%(SERVER, self.dump_dict[key]["BIRTHDAY"],self.dump_dict[key]["BIRTHDAY"]))
				else:
					self.table.setc(i+line, 5, self.dump_dict[key]["BIRTHDAY"].replace("data not available", "?"))


				self.table.setc(i+line, 6, self.dump_dict[key]["BONITATION"].replace("data not available", "?"))
				...
				self.table.setc(i+line, 10, self.dump_dict[key]["ED"].replace("data not available", "?"))
				if self.dump_dict[key]["ED"] != "data not available":
					...
					if "3" in ed_val:
						self.table.setca(i+line, 10,{'bgcolor':"#992222"})			
					if "2" in ed_val:
						self.table.setca(i+line, 10,{'bgcolor':"#994455"})
					if "1" in ed_val:
						self.table.setca(i+line, 10,{'bgcolor':"#886622"})

				# coi
				self.table.setc(i+line, 11, self.dump_dict[key]["COI5G"].replace("data not available", "?"))
				if self.dump_dict[key]["COI5G"] == "0.0000000000%":
				...
				if self.dump_dict[key]["BREEDER"]!="data not available":
					self.table.setc(i+line, 13, "<a href=%s/find_breeders.py?name=%s>%s</a>"%(SERVER, self.dump_dict[key]["BREEDER"].replace(" ", "%20"), self.dump_dict[key]["BREEDER"]))
				else:
